...
const AuditLogs = () => {
    ...
    const getActionClass = (action) => {
        if (action.includes('created')) return 'text-blue-500 bg-blue-100';
        if (action.includes('deleted')) return 'text-red-500 bg-red-100';
        if (action.includes('updated')) return 'text-yellow-500 bg-yellow-100';
        if (action.includes('login')) return 'text-green-500 bg-green-100';
        if (action.includes('sent')) return 'text-purple-500 bg-purple-100';
    ...
    const formatDetails = (log) => {
        ...
        if (action.includes('submission.viewed')) {
            if (details.params?.id) return `Submission ${details.params.id} viewed.`;
            return 'Submission viewed.';
        }
        
        // Fallback for other actions
        if (details.body && Object.keys(details.body).length > 0) {
            return `Body: ${JSON.stringify(details.body).substring(0, 50)}...`;
        }
        if (details.params && Object.keys(details.params).length > 0) {
            return `Params: ${JSON.stringify(details.params)}`;
        }
